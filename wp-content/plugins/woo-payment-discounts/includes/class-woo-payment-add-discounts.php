...
class Woo_Payment_Add_Discounts
{
    public function __construct()
    {
        ...
        add_action(
            ...
            [$this, "wpd_add_discount"],
            10
        );
        add_filter(
            "woocommerce_gateway_title",
            [$this, "wpd_payment_method_title"],
            10,
            2
        );
        add_action(
            "woocommerce_checkout_order_processed",
            [$this, "wpd_update_order_data"],
            10
        );
    }
    ...
    {
		$metoda=$gateway->title;
        if (strstr($value, "%")) {
            return sprintf(
                __("%s (%s off)", "woo-payment-discounts"),
                esc_attr($gateway->title),
                $value
            );
        }
			if($metoda!="PayPal"){
				return sprintf(
            __("%s ", "woo-payment-discounts"),
            esc_attr($gateway->title)
        )."mit 2% Skonto";
			}
		else{
    ...
    public function wpd_payment_method_title($title, $id)
    {
        ...
        if (isset($settings[$id]["amount"])) {
            ...
            //$title .= ' <small>(' . sprintf( __( '%s Henryk', 'woo-payment-discounts' ), $value ) . ')</small>';
            if ($discount > 0) {
                $title .=
                    " <small>(" .
                    sprintf(
                        __("Vorkasse - mit %s Skonto", "woo-payment-discounts"),
                        $value
                    ) .
                    ")</small>";
            } else {
                $title .=
                    " <small>(" .
                    sprintf(__("ohne Abzug", "woo-payment-discounts"), $value) .
                    ")</small>";
            }
        }
    ...
    public function wpd_add_discount($cart)
    {
        ...
        if (isset($gateways[WC()->session->chosen_payment_method])) {
            ...
            if (apply_filters("woo_payment_discounts_apply_discount", $cart)) {
                $payment_gateways = WC()->payment_gateways->payment_gateways();
                $gateway =
                    $payment_gateways[WC()->session->chosen_payment_method];
    ...
    {
        $payment_method_title = get_post_meta(
            $order_id,
            "_payment_method_title",
            true
        );
        $new_payment_method_title = preg_replace(
            "/<small>.*<\/small>/",
            "",
            $payment_method_title
        );
        // Save the new payment method title.
        $new_payment_method_title = sanitize_text_field(
